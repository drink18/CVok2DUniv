...
void cvPGSSolver::solvePenetrations(bool warmStart)
{
    ...
    for(int i = 0; i < m_ContactContraints.size(); ++i)
    {
        ...
        if(c.bodyAId >= 0)
            m_solverBodies[c.bodyAId].applyVelImp(c.JA, c.MA, lambda);

        if(c.bodyBId >= 0)
            m_solverBodies[c.bodyBId].applyVelImp(c.JB, c.MB, lambda);
    }
}
...
void cvPGSSolver::solvePositionErr()
{
    for(auto& c : m_ContactContraints)
    {
        ...
        if(c.bodyAId >= 0)
            m_solverBodies[c.bodyAId].applyPosImp(c.JA, c.MA, lambda);

        if(c.bodyBId >= 0)
            m_solverBodies[c.bodyBId].applyPosImp(c.JB, c.MB, lambda);
    }
}
...
void cvPGSSolver::solveFriction(bool warmStart)
{
    for(auto& c : m_ContactContraints)
    {
        ...
        if(c.bodyAId >= 0)
        {
            m_solverBodies[c.bodyAId].applyVelImp(c.tJA, c.MA, lambda);
        }

        if(c.bodyBId >= 0)
        {
            m_solverBodies[c.bodyBId].applyVelImp(c.tJB, c.MB, lambda);
        }
    }
}

void cvPGSSolver::solveRollingFriction()
{
    for(auto& c : m_ContactContraints)
    {
        cvVec3f velA, velB;
        ...
        if(c.bodyAId >= 0)
        {
            m_solverBodies[c.bodyAId].applyVelImp(c.rJA, c.MA, rlambda);
        }

        if(c.bodyBId >= 0)
        {
            m_solverBodies[c.bodyBId].applyVelImp(c.rJB, c.MB, rlambda);
        }
    }
}
